/*
	...
	How to use: 
		connect to pdb database using sqlplus or sqlcl. Privileges are required to access XDBMONITORING schema and gv$ views
...
select TRIM(sample_id) finish_sample_id_br from XDBMONITORING.tbl_snapshot where sample_id = &blendsnap;
select TRIM(sample_id) start_sample_id_tr from XDBMONITORING.tbl_snapshot where sample_id = &trstartsnap;
select TRIM(sample_id) finish_sample_id_tr from XDBMONITORING.tbl_snapshot where sample_id = &trendsnap;
select TO_CHAR(sample_time,'DD/MM/YYYY HH24:MI') start_sample_time_br from XDBMONITORING.tbl_snapshot where sample_id = &start_sample_id_br;
select TO_CHAR(sample_time,'DD/MM/YYYY HH24:MI') finish_sample_time_br from XDBMONITORING.tbl_snapshot where sample_id = &finish_sample_id_br;
...
prompt      <h5>
prompt      <span style="text-align:left;">for Oracle 12c - 19c XE/SE editions (based on xdbmonitoring schema)</span>
...
from XDBMONITORING.tbl_sys_time_model ss
...
from XDBMONITORING.tbl_sysstat ss
...
left join (select wait_class, event, time_waited_micro from XDBMONITORING.tbl_system_event where sample_id = &start_sample_id_br) es ON en.wait_class = es.wait_class AND en.name = es.event
left join (select wait_class, event, time_waited_micro from XDBMONITORING.tbl_system_event where sample_id = &finish_sample_id_br) ef ON en.wait_class = ef.wait_class AND en.name = ef.event
where ROUND((NVL(ef.time_waited_micro,0) - NVL(es.time_waited_micro,0))/1000000,1) > 0)
, tr as (
select en.wait_class, en.name, ROUND((NVL(ef.time_waited_micro,0) - NVL(es.time_waited_micro,0))/1000000,1) time_delta_min  from v$event_name en
left join (select wait_class, event, time_waited_micro from XDBMONITORING.tbl_system_event where sample_id = &start_sample_id_tr) es ON en.wait_class = es.wait_class AND en.name = es.event
left join (select wait_class, event, time_waited_micro from XDBMONITORING.tbl_system_event where sample_id = &finish_sample_id_tr) ef ON en.wait_class = ef.wait_class AND en.name = ef.event
where ROUND((NVL(ef.time_waited_micro,0) - NVL(es.time_waited_micro,0))/1000000,1) > 0)
...
, tr as (select stat_name, max(value)-min(value) value from XDBMONITORING.TBL_SYS_TIME_MODEL where sample_id between &start_sample_id_tr and &finish_sample_id_tr group by stat_name)
, br as (select stat_name, max(value)-min(value) value from XDBMONITORING.TBL_SYS_TIME_MODEL where sample_id between &start_sample_id_br and &finish_sample_id_br group by stat_name)
...
FROM tr
LEFT JOIN s  ON tr.sql_id_t=s.sql_id
INNER JOIN XDBMONITORING.TBL_SQL_TEXT t ON tr.sql_id_t=t.sql_id )
...
, bl as (select /*+ materialize */ name, max(value) - min(value) value from XDBMONITORING.tbl_sysstat where sample_id between &start_sample_id_br and &finish_sample_id_br group by name)
, tr as (select /*+ materialize */ name, max(value) - min(value) value from XDBMONITORING.tbl_sysstat where sample_id between &start_sample_id_tr and &finish_sample_id_tr group by name)
