/*
 -- XDBTIME - Database Performance - Period Comparison Report (based on AWR data)
 -- Copyright (c) 2016, 2020, XDBTIME.  All rights reserved.
 -- filename: xdbtimeorclawr.sql

 -- 
 -- description: SQL script to generate Database Performance Comparison HTML report for 2 time periods for Oracle Database
 -- database: Oracle 12c-19c Enterprise Editions (EE, EE-HP, EE-EP) with Diagnostic Package License (AWR).
 -- how to use: 
 -- 	connect to cdb/pdb database. Privileges are required to access cdb_hist and gv$ views
 ...
 -- 				  You can use awrlistdbid.sql to list available dbid in AWR
 -- 				  You can use awrlistsnap.sql to list snapshots 
 -- example: 
 ...
 -- 	Report is running ...
 -- 	Report is written to /Users/xdbtime/Reports/XDBT_DB0103_189780284_1_1to15vs2185458275_1_1to16.html
*/
set echo off;
set pages 0;
set veri off;
...
select dbid db_id_br, instance_number inst_id_br, snap_id start_sample_id_br from cdb_hist_snapshot where dbid = &bldbid and instance_number = &blinstance_number and snap_id = &blstartsnap;
select dbid db_id_br, instance_number inst_id_br, snap_id finish_sample_id_br from cdb_hist_snapshot where dbid = &bldbid and instance_number = &blinstance_number and snap_id = &blendsnap;
select dbid db_id_tr, instance_number inst_id_tr, snap_id start_sample_id_tr from cdb_hist_snapshot where dbid = &trdbid and instance_number = &trinstance_number and snap_id = &trstartsnap;
select dbid db_id_tr, instance_number inst_id_tr, snap_id finish_sample_id_tr from cdb_hist_snapshot where dbid = &trdbid and instance_number = &trinstance_number and snap_id = &trendsnap;
...
GROUP BY SQL_ID,PLAN_HASH_VALUE))
, s as (select 'sqlid1' sql_id, 'sql_type1' sql_type from dual union
